...
app.post('/api/cnj', (req, res) => {
    ...
    function getEndPointApi(processo) {
        ...
        const j = parseInt(processo.slice(13, 14));
        const tr = parseInt(processo.slice(14, 16));
    ...
    const apiInfo = getEndPointApi(processo);

    // The key is publicly available in CNJ documentation
    const options = {
        method: 'POST',
        headers: {
...
// Start
app.listen(PORT, () => {
    console.log(`CNJ backend running in: http://localhost:${PORT}`);
});
